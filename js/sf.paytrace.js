...
sf.paytrace.processProtectAuth = function(hpfToken, encKey, ptProtectToken) {
    ...
    ptForm = document.getElementById("ProtectForm"),
	  pfFormSubmitButton = $('#' + sf.paytrace.buttons.submit.id).val(),
    pfFormCancelButton = $('#' + sf.paytrace.buttons.cancel.id).val(),
    pfFormFinishButton = $('#' + sf.paytrace.buttons.finish.id).val(),
    detail = '',
    description = '';
    ...
    $.ajax({
      ...
      success: function(data, status, jqXHR) {
        ...
        if(responseCode == 101) {
        	...
        	pfFormSubmitButton.addClass('hide');
   		 	pfFormCancelButton.addClass('hide');
    		pfFormFinishButton.removeClass('hide');
        }
        else {
...
sf.paytrace.finishPtProtect = function() {
	var formOuter = $('#pt_protect_form_outer');
...
sf.paytrace.initPtProtect = function(f_success, f_fail) {
  if(!sf.paytrace.state.init) {
    ...
    var overlay = document.getElementById('pt_overlay'),
        documentBody = $(document.body),
        formOuter = $('#pt_protect_form_outer'),
    	ptProtectToken = '',
        amount = $('#input_10_10').val();
    
    $('#pt_form_message').text('');
	...
	$.post(sf.paytrace.baseEndpointUri + 'getPtProtectToken.php', request, 
    function (data) {
      // DEBUG
...
var ptForm = document.getElementById(sf.paytrace.forms.protect.id),
	  pfFormSubmitButton = document.getElementById(sf.paytrace.forms.protect.buttons.submit),
    pfFormCancelButton = document.getElementById(sf.paytrace.forms.protect.buttons.cancel),
    pfFormFinishButton = document.getElementById(sf.paytrace.forms.protect.buttons.finish);
